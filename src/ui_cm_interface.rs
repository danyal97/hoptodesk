            ...
            tokio::select! {
                res = self.stream.next() => {
                    match res {
                        ...
                        Ok(Some(data)) => {
                            match data {
                                Data::Login{id, is_file_transfer, is_view_camera, port_forward, peer_id, name, authorized, keyboard, clipboard, audio, file, file_transfer_enabled: _file_transfer_enabled, restart, recording, block_input, from_switch, security_numbers, avatar_image} => {
                                    log::debug!("conn_id: {}", id);
                                    self.cm.add_connection(id, is_file_transfer, is_view_camera, port_forward, peer_id, name, authorized, keyboard, clipboard, audio, file, restart, recording, block_input, from_switch, self.tx.clone(), security_numbers, avatar_image);
                                    self.conn_id = id;
                                    #[cfg(target_os = "windows")]
                                    {
